...
public interface UserProfileRepo extends JpaRepository<UserProfileEntity, Long> {
    ...
    @Query(value = "SELECT * FROM user_profile ORDER BY RAND()", nativeQuery = true)
    Page<UserProfileEntity> findUsersByRandomAndPage(Pageable pageable);

    @Query(value = "select up.user_id as userId, up.first_name as firstName, up.last_name as lastName, up.email as email, up.experience as experience, up.date_of_birth as dateOfBirth, up.profile_picture_url as profilePictureUrl,\n" +
